...
try:
    ...
    def carregar_agendamentos():
        try:
            agendamentos = pd.DataFrame(sheet.get_all_records())
            return agendamentos if {'Data', 'Horário', 'Nome'}.issubset(agendamentos.columns) else pd.DataFrame(columns=['Data', 'Horário', 'Nome'])
        except Exception as e:
            st.error(f"Erro ao carregar agendamentos: {e}")
            return pd.DataFrame(columns=['Data', 'Horário', 'Nome'])

    # Validar disponibilidade e agendar
    def validar_e_agendar(data, horario, nome, agendamentos):
        if agendamentos[(agendamentos['Data'] == data) & (agendamentos['Horário'] == horario)].empty:
            if nome not in agendamentos['Nome'].values:
                sheet.append_row([data, horario, nome])
                st.success(f"Entrevista agendada para {nome} no dia {data} às {horario}!")
                return True
            st.error("Este nome já foi utilizado. Por favor, use um nome diferente.")
        else:
            st.error(f"Horário {horario} no dia {data} já agendado!")
        return False
    ...
    horarios_disponiveis = [h for h in horarios_por_data[data] if agendamentos[(agendamentos['Data'] == data) & (agendamentos['Horário'] == h)].empty]
    if horarios_disponiveis:
        horario = st.selectbox("Escolha o horário:", horarios_disponiveis)
        if st.button("Confirmar Agendamento") and len(nome.split()) >= 2:
            validar_e_agendar(data, horario, nome.strip(), agendamentos)
    else:
        st.warning("Todos os horários para esta data estão ocupados. Escolha outra data.")

except Exception as e:
    st.error(f"Erro ao configurar credenciais ou acessar o Google Sheets: {e}")
